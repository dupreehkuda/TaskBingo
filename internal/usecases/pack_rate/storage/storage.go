...
// Rate increments packs.rating and appends the pack to users.ratedpacks.
// Caller must wrap in pgxtx.RunInTx.
func (s *Storage) Rate(ctx context.Context, userID, packID string) error {
	...
	_, err := s.q(ctx).Exec(ctx,
		`UPDATE users SET ratedpacks = ARRAY_APPEND(ratedpacks, $1) WHERE id = $2`,
		packID, userID,
	)
	return err
}
